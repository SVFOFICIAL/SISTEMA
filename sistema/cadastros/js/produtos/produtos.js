...
export const prod = {
    ...
    $('#btn_inativar').click(function(){
        ...
        $('#produtos').on('click', '.atualizar_prod_dest', function(e){
            var idprod = $(e.currentTarget).data('idprod');
            let url = $(e.currentTarget).data('url');
            $.ajax({
              url: url + '/controllers/produto.php?action=pfd',
              method: "post",
              data: {'iditem' : idprod, lote : false},
     ...
     $('#categoria').change( function(){
        ...
        $('#produtos_inativos').change( function(){
            ...
            if(!$(this).is(':checked')){           
                ...
                prod.table_prod.columns(7).search('Sim').draw();
       ...
       $('#produtos_destaque').change( function(){
          
            if(!$(this).is(':checked')){           
            
                prod.table_prod.column(6).search('').draw();      
                                
            }else {
                //prod.table_prod.column(6).data().filter(function(value, index){ return value == "Não"}.draw())
                prod.table_prod.column(6).search('Sim').draw();               
            }                
         })      
