...
if(!$lerbanco->getResult()){
	...
	if (!empty($img_item) && $img_item != "null" && file_exists(UPLOAD_PATH.$img_item) && !is_dir(UPLOAD_PATH.$img_item)){									
		if($detect->isMobile()){
			echo "<div   data-target=\"#popuppedido_$id\" data-toggle=\"modal\" class=\"bg-white\"><figure style=\"margin:10px\" class=\"destaque_prod\">".Check::Image($img_item, 'Imagem-item', 100, 100)."</figure><div   class=\"flex flex-col\"> <div class=\"w-full\"><span style=\"margin:10px; cursor:pointer;\">$nome_item</span></div><div class=\"w-full\"><span style=\"margin:10px; cursor:pointer;\">R$ ".Check::real($preco_item)."</span></div></div></div>";
										...
										<form data-url="<?=$site?>" id="addItemPost_<?=$ido_DoItem;?>" name="addItemPost_<?=$ido_DoItem;?>" method="post">
											<td class="options">
																	...
																	<div style="margin-top:0px !important"class="w-full flex flex-col dropdown-menupop">
																		...
																		<?php
																			if (!empty($img_item) && $img_item != "null" && file_exists(UPLOAD_PATH.$img_item) && !is_dir(UPLOAD_PATH.$img_item)):										
																				...
																				echo "<div class=\"flex w-full\"> <img width=\"100%\" height=\"\" src=\"{$urlImage}\" /></div>";
																		endif;
																		?>
																		...
																		$lerbanco->ExeRead("ws_relacao_tamanho", "WHERE id_user = :useriid AND id_item = :idiitem", "useriid={$getu}&idiitem={$ido_DoItem}");
																		if(!$lerbanco->getResult()):
																			echo "<div style=\"margin-top:20px;margin-bottom:20px;\" class=\"w-full\"><span>R$ ".Check::real($preco_item)."</span></div>";
																		else:
																			echo "<div style=\"margin-top:20px;margin-bottom:20px;\" class=\"w-full\"><span'>Escolha uma opção abaixo</span></div>>";
																		endif;
												...
												// INICIO DAS OPÇÕES DE TAMANHO
																	...
																	if(!$lerbanco->getResult()):
																		echo "<input id=\"valor_item\" type=\"hidden\" name=\"valor\" value=\"{$preco_item}\">";
																	else:
...
<div class="flex flex-col w-full">
	...
	<div style="height: 50%;" class="w-full "> 
		...
		<div style="color:white;background: #46DC4C;height: 26px" class="flex justify-center w-full">
			<button data-vladicional="<?=$valor_adicional?>" data-idtipo="<?=$id_tipo?>" data-iditem="<?=$ido_DoItem;?>" type="button" class="btn-number btn-plus_<?=$id_tipo?>"  data-maxadcionais="<?=$quantidade?>" data-idadicional="<?=$id_adicionais?>" data-type="plus" data-field="quantidade">
...
if(!empty($disponivel) && $disponivel == 1):
	?>	
	<div style="border-radius: 0;padding: unset;border-top: unset;bottom: 0px;position: sticky;width: 100%;" class="modal-footer">
	...
	<div style="background: #FF0000" class="w-1/6 items-center lex-row flex">
		...
		<div style="background: #46DC4C;padding:10px;height:100%" class="w-full items-center flex flex-row">
			<div style="font-size: 25px;" class="text-left w-full">
				...
				<span>R$: </span>
				<span data-valoritem="<?=Check::real($preco_item)?>" id="valorItem_<?=$ido_DoItem;?>"><?=Check::real($preco_item)?></span>	
				</div>
						...
						if(j.success && !j.errror){
							x0p('Sucesso!', 
								...
								var sound = new Howl({
									...
									$("#total_pedido").text("R$: "+ parseFloat(j.total_pedido).toFixed(2))
...
<div class="modal fade cart-modal" id="cart-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="theiaStickySidebar">
		...
		<div  class="config-header w-full text-bold text-center text-white">
				...
				<div id="limpaCarrinho" style="margin-bottom:50px; display:none" class="w-full mt-3">
			...
			<form data-toggle="validator" action="<?=$site.$Url[0].'/';?>carrinho" method="post">
				...
				<table style="display: none" class="table table_summary">
					...
					<div id="checkout" style="border-radius: 0;padding:unset; border-top: unset;position:absolute;width:100%;" class="modal-footer">
<div  class="flex flex-row w-full" style="height: 100%;">
<?php $total_do_pedido = $cart->getAttributeTotal('preco') + $totaldeadicionais; ?>
	<div style="background: #FF0000" class="w-1/6 items-center lex-row flex">
			<a data-iditem="<?=$ido_DoItem;?>" data-dismiss="modal" style="padding: 10px; cursor: pointer;"id="fecharmodallogin" class="close-link">
		...
		<button type="submit" style="cursor: pointer;" id="checkout">
		<div style="background: #46DC4C;padding:10px;height:100%" class="w-full items-center flex flex-row">
			<div style="font-size: 20px;" class="text-left w-full">
			...
			<div style="display:none" data-toggle="modal" data-target="#cart-modal"class="w-full flex cursor-pointer" id="totalizador"  >
					<?php $total_do_pedido = $cart->getAttributeTotal('preco') + $totaldeadicionais; ?>
					...
					<div  style="padding:5px" class="">
									<span id="total_pedido" style="color:white" class="pull-right">
										 RS:  <!-- <span id="v-total-p"> -->
											<?php
											if(!empty($_SESSION['desconto_cupom']) && $_SESSION['desconto_cupom']['user_id'] == $getu):
												$valordesconto = Check::porcentagem($_SESSION['desconto_cupom']['desconto'], $total_do_pedido);
												echo Check::Real($total_do_pedido - $valordesconto);
