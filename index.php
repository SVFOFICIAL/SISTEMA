...
if(empty($Url[0]) || $Url[0] == 'index'):
	...
	<head>
		...
		<link href="<?= $site; ?>css/scrollmenu.css" rel="stylesheet">
...
.form-control:focus{
	...
	outline: 0 !important;
	-webkit-box-shadow: inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px #7233A1 !important;
	box-shadow: inset 0 1px 1px rgba(0,0,0,.075), 0 0 8px #7233A1 !important;
			...
			if(currentVal > input.attr('min')) {
				...
				if(adicional && valorAdicional && idItem && parseFloat(valorAdicional)>0){
					...
					if(sumValorAdicionais==0 && sumValorAdicionaisTotal==0 && adicionais=='false'){
						...
						$("#valorItem_"+idItem).attr('data-add', false);
						$("#valorItem_"+idItem).attr('data-newvlritem', valorAtualizado);
						priceItem = [];
					...
					if(qtMaxTiposAdicionais && sumValorAdicionais >= qtMaxTiposAdicionais){
						...
						let valorAtualItem = priceItem.length >0 ? priceItem.pop() : parseFloat($("#valorItem_"+idItem).attr('data-valoratual'));
						 
						let valorAtualItemTotal=  parseFloat($("#valorItem_"+idItem).attr('data-valoratual'));
						valorAdicional = parseFloat(valorAdicional);
					
					
					let valorAtualizadoItem = valorAtualItem-valorAdicional;
					let valorAtualizadoTotalItem = valorAtualItemTotal-valorAdicional;
				 
					priceItem.push(valorAtualizadoItem)

					$('#valorItem_'+idItem).text(parseFloat(valorAtualizadoTotalItem).toFixed(2));
					$("#valorItem_"+idItem).attr('data-newvlritem', valorAtualizadoItem);
					$("#valorItem_"+idItem).attr('data-valoratual', valorAtualizadoTotalItem)
					$("#valor_item").val(parseFloat(valorAtualizadoTotalItem).toFixed(2))
			...
			if(currentVal < input.attr('max')) {
				...
				if(adicional && valorAdicional && idItem &&parseFloat(valorAdicional)>0){
					...
					if(qtMaxTiposAdicionais && sumValorAdicionais >= qtMaxTiposAdicionais){
						...
						let valorAtualItem = priceItem.length >0 ? priceItem.pop() : parseFloat($("#valorItem_"+idItem).attr('data-valoratual'));
						let valorTotalItem = parseFloat($("#valorItem_"+idItem).attr('data-valoratual'));
				
						
					 
					valorAdicional = parseFloat(valorAdicional);
					let valorAtualizadoItem = valorAtualItem+valorAdicional;
					let valorAtualizadoTotalItem = valorTotalItem + valorAdicional;
					priceItem.push(valorAtualizadoItem);
					$("#valorItem_"+idItem).attr('data-valoratual', valorAtualizadoTotalItem)
					$("#valorItem_"+idItem).attr('data-newvlritem', valorAtualizadoItem);
			...
			if(currentVal > input.attr('min')) {
				...
				if(input){
					...
					if(sumValorAdicionais==0 && sumValorAdicionaisTotal==0 && adicionais=='false'){
						...
						let valorItem = parseFloat($("#valorItem_"+idItem).attr('data-newvlritem'));
						let valorAtual =parseFloat($("#valorItem_"+idItem).attr('data-valoratual'));
						let valorAtualizado = valorAtual - valorItem;
			...
			if(currentVal < input.attr('max')) {
				...
				if(input){
					...
					if(sumValorAdicionais==0 && sumValorAdicionaisTotal==0 && adicionais=='false'){
						...
						let valorAtual =parseFloat($("#valorItem_"+idItem).attr('data-newvlritem'));
						let valorAtualizado = valorAtual *qtItem;
